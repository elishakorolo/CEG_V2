...
function displayBeamResults(data, designData, barDia) {
    ...
    // ── BS 8110 DESIGN CARDS (light theme) ─────────────────────────────────────
    if (designData && designData.success) {
        ...
        // Header
        html += `
        <div style="background:#f0f4ff;border-left:4px solid #4f46e5;border-radius:8px;
                    padding:12px 16px;margin-bottom:14px;font-size:13px;">
            <div style="font-weight:700;color:#3730a3;margin-bottom:4px;">${code}</div>
            <div style="color:#6b7280;font-size:12px;margin-bottom:8px;">
                Nigerian standard per NIStructE / COREN &nbsp;|&nbsp;
                &#947;m = 1.5 (concrete) &nbsp;|&nbsp; &#947;s = 1.05 (steel)
            </div>
            <div style="display:flex;flex-wrap:wrap;gap:14px;font-size:12px;color:#374151;">
                <span><span style="color:#6b7280;">Section:</span> <b>${b_mm}&times;${h_mm} mm</b></span>
                <span><span style="color:#6b7280;">d<sub>eff</sub>:</span> <b>${d_eff} mm</b></span>
                <span><span style="color:#6b7280;">Cover:</span> <b>${cov} mm</b></span>
                <span><span style="color:#6b7280;">fcu:</span> <b>${fcu} MPa</b></span>
                <span><span style="color:#6b7280;">fy:</span> <b>${fy} MPa</b></span>
                <span><span style="color:#6b7280;">fyv:</span> <b>${fyv} MPa</b></span>
            </div>
        </div>`;

        // ── per-face card helper (light theme, no icons) ───────────────────────
        function faceCard(face, label, labelFull, accentBorderColor, accentTextColor) {
            ...
            let out = `
            <div style="background:white;border:1px solid #e5e7eb;border-left:4px solid ${accentBorderColor};
                        border-radius:8px;padding:14px 16px;margin-bottom:12px;
                        box-shadow:0 1px 4px rgba(0,0,0,0.06);">
                <div style="font-size:13px;font-weight:700;color:${accentTextColor};margin-bottom:12px;">
                    ${label} <span style="font-weight:400;color:#6b7280;font-size:11px;">(${labelFull})</span>
                </div>
                <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-bottom:12px;">
                    <div><div style="font-size:10px;color:#6b7280;margin-bottom:2px;">M design (kNm)</div>
                         <div style="font-size:18px;font-weight:700;color:#111;">${M_d}</div></div>
                    <div><div style="font-size:10px;color:#6b7280;margin-bottom:2px;">As,req (mm&sup2;)</div>
                         <div style="font-size:18px;font-weight:700;color:${accentTextColor};">${As_req}</div></div>
                    <div><div style="font-size:10px;color:#6b7280;margin-bottom:2px;">As,prov (mm&sup2;)</div>
                         <div style="font-size:18px;font-weight:700;color:#111;">${As_prov}</div></div>
                    <div><div style="font-size:10px;color:#6b7280;margin-bottom:2px;">Mu,cap (kNm)</div>
                         <div style="font-size:18px;font-weight:700;color:#111;">${Mu_cap}</div></div>
                    <div><div style="font-size:10px;color:#6b7280;margin-bottom:2px;">Utilization</div>
                         <div style="font-size:18px;font-weight:700;color:${utilColor};">${utilPct.toFixed(1)}%</div></div>
                </div>
                <div style="background:#f3f4f6;border-radius:4px;height:8px;overflow:hidden;margin-bottom:10px;">
                    <div style="width:${utilPct.toFixed(1)}%;background:${utilColor};height:100%;border-radius:4px;"></div>
                </div>
                <table style="width:100%;border-collapse:collapse;font-size:12px;color:#374151;margin-bottom:10px;">
                    <tr style="border-bottom:1px solid #f3f4f6;">
                        <td style="padding:4px 0;color:#6b7280;">K = M / (fcu&middot;b&middot;d&sup2;)</td>
                        <td style="text-align:right;font-weight:700;color:${K_ok?PASS:FAIL};">
                            ${K} ${K_ok?'&le;':'>'} K'=${K_lim} ${K_ok?'(OK)':'(FAIL)'}
                        </td>
                    </tr>
                    <tr style="border-bottom:1px solid #f3f4f6;">
                        <td style="padding:4px 0;color:#6b7280;">Section type</td>
                        <td style="text-align:right;font-weight:600;">${face.section_type || 'Singly Reinforced'}</td>
                    </tr>
                    <tr style="border-bottom:1px solid #f3f4f6;">
                        <td style="padding:4px 0;color:#6b7280;">Lever arm z</td>
                        <td style="text-align:right;">${z} mm</td>
                    </tr>
                    <tr>
                        <td style="padding:4px 0;color:#6b7280;">Status</td>
                        <td style="text-align:right;font-weight:700;color:${statusOk?PASS:FAIL};">${status}</td>
                    </tr>
                </table>`;
            if (bars.length > 0) {
                out += `<div style="font-size:11px;color:#374151;font-weight:600;margin-bottom:4px;">
                    Bar options (min Y16, Nigerian norm):</div>
                <table style="width:100%;border-collapse:collapse;font-size:12px;">
                    <thead><tr style="background:#f9fafb;">
                        <th style="padding:4px 8px;text-align:left;border-bottom:1px solid #e5e7eb;color:#6b7280;">Bars</th>
                        <th style="padding:4px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">As prov (mm&sup2;)</th>
                        <th style="padding:4px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">Excess (mm&sup2;)</th>
                    </tr></thead><tbody>`;
                bars.forEach((b_opt, idx) => {
                    out += `<tr style="border-bottom:1px solid ${idx<bars.length-1?'#f3f4f6':'transparent'};">
                        <td style="padding:4px 8px;font-weight:700;color:${accentTextColor};">${b_opt.label}</td>
                        <td style="padding:4px 8px;text-align:right;">${b_opt.area}</td>
                        <td style="padding:4px 8px;text-align:right;color:#9ca3af;">+${b_opt.excess}</td>
                    </tr>`;
                });
                out += `</tbody></table>`;
            }
        ...
        if (hasNewFormat) {
            html += faceCard(bot, 'BOTTOM STEEL', 'Sagging / positive moment', '#16a34a', '#15803d');
            html += faceCard(top, 'TOP STEEL',    'Hogging / negative moment', '#dc2626', '#b91c1c');
        } else {
            const legacyFace = {
                M_design: data.max_moment || 0, K: uls.K_actual || 0, K_lim: uls.K_limit || 0.156,
                z: d_eff * 0.9, As_req: uls.steel_area_req || 0, As_prov: uls.steel_area_prov || 0,
                Mu_cap: uls.moment_capacity || 0, util: (uls.utilization || 0) / 100,
                status: 'OK', section_type: uls.section_type || 'Singly Reinforced',
                bar_options: designData.bar_options || [],
            };
            html += faceCard(legacyFace, 'BOTTOM STEEL', 'Sagging', '#16a34a', '#15803d');
        }

        // ── Shear ─────────────────────────────────────────────────────────────
        const shearData = hasNewFormat ? shr : {
            v: uls.shear_stress_actual, vc: uls.shear_stress_allow, v_max: 5.0, vc_kN: 0,
            status: uls.shear_check || '', util: uls.shear_util || 0,
            link_options: uls.link_options || [], V_design: 0, sv_max: uls.sv_max || 0
        };
        if (shearData && shearData.v !== undefined) {
            ...
            html += `
            <div style="background:white;border:1px solid #e5e7eb;border-left:4px solid #d97706;
                        border-radius:8px;padding:14px 16px;margin-bottom:12px;
                        box-shadow:0 1px 4px rgba(0,0,0,0.06);">
                <div style="font-size:13px;font-weight:700;color:#92400e;margin-bottom:12px;">
                    SHEAR DESIGN <span style="font-weight:400;color:#6b7280;font-size:11px;">(BS 8110 Cl.3.4.5)</span>
                </div>
                <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:10px;">
                    <div><div style="font-size:10px;color:#6b7280;margin-bottom:2px;">V design (kN)</div>
                         <div style="font-size:18px;font-weight:700;color:#111;">${(shearData.V_design||0).toFixed(2)}</div></div>
                    <div><div style="font-size:10px;color:#6b7280;margin-bottom:2px;">v = V/(b&middot;d)</div>
                         <div style="font-size:18px;font-weight:700;color:#111;">${v_v} N/mm&sup2;</div></div>
                    <div><div style="font-size:10px;color:#6b7280;margin-bottom:2px;">vc (concrete)</div>
                         <div style="font-size:18px;font-weight:700;color:#111;">${vc_v} N/mm&sup2; (${vc_kN} kN)</div></div>
                    <div><div style="font-size:10px;color:#6b7280;margin-bottom:2px;">v,max = 0.8&radic;fcu</div>
                         <div style="font-size:18px;font-weight:700;color:#111;">${vmax_v} N/mm&sup2;</div></div>
                </div>
                <div style="background:#f3f4f6;border-radius:4px;height:8px;overflow:hidden;margin-bottom:6px;">
                    <div style="width:${sUtil.toFixed(1)}%;background:${shColor};height:100%;border-radius:4px;"></div>
                </div>
                <div style="font-size:11px;color:#6b7280;margin-bottom:${links.length?'10px':'0'};">
                    v/vc = ${sUtil.toFixed(1)}% &nbsp;|&nbsp;
                    <span style="color:${shColor};font-weight:700;">${st}</span>
                    &nbsp;|&nbsp; sv,max = ${(shearData.sv_max||0).toFixed(0)} mm
                </div>`;
            if (links.length > 0) {
                html += `<table style="width:100%;border-collapse:collapse;font-size:12px;margin-top:8px;">
                    <thead><tr style="background:#f9fafb;">
                        <th style="padding:4px 8px;text-align:left;border-bottom:1px solid #e5e7eb;color:#6b7280;">Size @ sv</th>
                        <th style="padding:4px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">Asv (mm&sup2;)</th>
                        <th style="padding:4px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">sv (mm)</th>
                        <th style="padding:4px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">v prov (N/mm&sup2;)</th>
                    </tr></thead><tbody>`;
                links.forEach((lk, idx) => {
                    html += `<tr style="border-bottom:1px solid ${idx<links.length-1?'#f3f4f6':'transparent'};">
                        <td style="padding:4px 8px;font-weight:700;color:#92400e;">${lk.label}</td>
                        <td style="padding:4px 8px;text-align:right;">${lk.Av}</td>
                        <td style="padding:4px 8px;text-align:right;">${lk.sv}</td>
                        <td style="padding:4px 8px;text-align:right;">${lk.v_prov}</td>
                    </tr>`;
                });
                html += `</tbody></table>`;
            }
        ...
        if (sls && sls.deflection_check) {
            ...
            <div style="background:white;border:1px solid #e5e7eb;border-radius:8px;
                ...
                <div style="font-size:13px;font-weight:700;color:#374151;margin-bottom:10px;">
                    SLS CHECKS <span style="font-weight:400;color:#6b7280;font-size:11px;">(Serviceability)</span>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                    <div style="padding:10px;background:${defOk?'#f0fdf4':'#fef2f2'};border-radius:6px;
                                border-left:3px solid ${defOk?PASS:FAIL};">
                        <div style="font-weight:700;color:${defOk?PASS:FAIL};font-size:12px;margin-bottom:4px;">
                            Deflection: ${defOk?'OK':'FAIL'}
                        </div>
                        <div style="font-size:12px;color:#374151;">
                            span/d = <b>${(sls.span_depth_actual||0).toFixed(1)}</b>
                            &nbsp;(limit ${(sls.span_depth_allowable||0).toFixed(1)})
                        </div>
                        <div style="font-size:11px;color:#6b7280;margin-top:2px;">
                            Util: ${(sls.deflection_util||0).toFixed(0)}%
                        </div>
                    </div>
                    <div style="padding:10px;background:${crOk?'#f0fdf4':'#fef2f2'};border-radius:6px;
                                border-left:3px solid ${crOk?PASS:FAIL};">
                        <div style="font-weight:700;color:${crOk?PASS:FAIL};font-size:12px;margin-bottom:4px;">
                            Crack Control: ${crOk?'OK':'Review'}
                        </div>
                        <div style="font-size:12px;color:#374151;">
                            Spacing = <b>${(sls.actual_spacing||0).toFixed(0)} mm</b>
                            &nbsp;(max ${(sls.max_bar_spacing||0).toFixed(0)} mm)
                        </div>
                        <div style="font-size:11px;color:#6b7280;margin-top:2px;">
                            fs = ${(sls.service_stress||0).toFixed(0)} MPa
                        </div>
                    </div>
                </div>
            </div>`;
            ...
            <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;
                ...
                <table style="width:100%;border-collapse:collapse;font-size:12px;color:#374151;">
                    <tr style="border-bottom:1px solid #e5e7eb;">
                        <td style="padding:4px 0;color:#6b7280;width:40%;">Section</td>
                        <td style="padding:4px 0;font-weight:600;">${b_mm}&times;${h_mm} mm &nbsp; d = ${d_eff} mm</td>
                    </tr>
                    <tr style="border-bottom:1px solid #e5e7eb;">
                        <td style="padding:4px 0;color:#6b7280;">Materials</td>
                        <td style="padding:4px 0;font-weight:600;">C${fcu} (fcu=${fcu} MPa) &nbsp; Y${fy} &nbsp; R${fyv}</td>
                    </tr>
                    <tr style="border-bottom:1px solid #e5e7eb;">
                        <td style="padding:4px 0;color:#6b7280;">Cover</td>
                        <td style="padding:4px 0;font-weight:600;">${cov} mm</td>
                    </tr>
                    <tr style="border-bottom:1px solid #e5e7eb;">
                        <td style="padding:4px 0;color:#15803d;">Bottom steel</td>
                        <td style="padding:4px 0;font-weight:600;color:#15803d;">
                            ${botBest.label||'&mdash;'} &nbsp; As,prov = ${Math.round(bot.As_prov||0)} mm&sup2;
                            &nbsp; (req = ${Math.round(bot.As_req||0)} mm&sup2;)
                        </td>
                    </tr>
                    <tr style="border-bottom:1px solid #e5e7eb;">
                        <td style="padding:4px 0;color:#b91c1c;">Top steel</td>
                        <td style="padding:4px 0;font-weight:600;color:#b91c1c;">
                            ${topBest.label||'&mdash;'} &nbsp; As,prov = ${Math.round(top.As_prov||0)} mm&sup2;
                            &nbsp; (req = ${Math.round(top.As_req||0)} mm&sup2;)
                        </td>
                    </tr>
                    <tr style="border-bottom:1px solid #e5e7eb;">
                        <td style="padding:4px 0;color:#92400e;">Links (stirrups)</td>
                        <td style="padding:4px 0;font-weight:600;color:#92400e;">
                            ${lkBest.label||'R8@nominal'} &nbsp; (2-leg R-bars, mild steel)
                        </td>
                    </tr>
                    <tr style="border-bottom:1px solid #e5e7eb;">
                        <td style="padding:4px 0;color:#6b7280;">Mu+ (sagging)</td>
                        <td style="padding:4px 0;">
                            M = ${(bot.M_design||0).toFixed(2)} kNm &nbsp; &rarr; &nbsp;
                            <b style="color:#15803d;">Mu,cap = ${(bot.Mu_cap||0).toFixed(2)} kNm</b>
                        </td>
                    </tr>
                    <tr style="border-bottom:1px solid #e5e7eb;">
                        <td style="padding:4px 0;color:#6b7280;">Mu- (hogging)</td>
                        <td style="padding:4px 0;">
                            M = ${(top.M_design||0).toFixed(2)} kNm &nbsp; &rarr; &nbsp;
                            <b style="color:#b91c1c;">Mu,cap = ${(top.Mu_cap||0).toFixed(2)} kNm</b>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding:4px 0;color:#6b7280;">Shear</td>
                        <td style="padding:4px 0;">
                            v = ${(shr.v||0).toFixed(3)} N/mm&sup2; &nbsp; vc = ${(shr.vc||0).toFixed(3)} N/mm&sup2;
                            &nbsp; &rarr; &nbsp;
    ...
    <div style="background:#eff6ff;padding:12px 16px;border-radius:10px;border-left:4px solid #3b82f6;
        ...
        ${data.max_sagging !== undefined ? `<span style="color:#16a34a;font-size:12px;">+sagging: ${(data.max_sagging||0).toFixed(2)} kNm</span>` : ''}
        ${data.max_hogging !== undefined ? `<span style="color:#dc2626;font-size:12px;">-hogging: ${(data.max_hogging||0).toFixed(2)} kNm</span>` : ''}
    </div>`;
...
function displayFrameResults(data, memberDesigns, barDia) {
    ...
    // ── Per-member design cards ──────────────────────────────────────────────────
    const beamDesigns   = (memberDesigns || []).filter(d => d && d.type === 'beam');
    const columnDesigns = (memberDesigns || []).filter(d => d && d.type === 'column');

    // Helper: render a compact BS 8110 face row (bottom or top steel)
    function faceRow(face, rowLabel, accentColor) {
        if (!face || !face.As_req) return '';
        const K   = (face.K || 0).toFixed(4);
        const Kl  = (face.K_lim || 0.156).toFixed(3);
        const K_ok = parseFloat(K) <= parseFloat(Kl);
        const utilPct = Math.min((face.util || 0) * 100, 100);
        const uColor  = utilPct < 70 ? PASS : utilPct < 90 ? WARN : FAIL;
        const bestBar = (face.bar_options || [])[0] || {};
        return `
        <tr style="border-bottom:1px solid #f3f4f6;">
            <td style="padding:5px 8px;font-weight:700;color:${accentColor};">${rowLabel}</td>
            <td style="padding:5px 8px;text-align:right;">${(face.M_design||0).toFixed(2)}</td>
            <td style="padding:5px 8px;text-align:right;">
                <span style="color:${K_ok?PASS:FAIL};font-weight:600;">${K}</span>
                <span style="color:#9ca3af;font-size:10px;"> / ${Kl}</span>
            </td>
            <td style="padding:5px 8px;text-align:right;">${(face.z||0).toFixed(0)}</td>
            <td style="padding:5px 8px;text-align:right;color:${accentColor};font-weight:600;">${Math.round(face.As_req||0)}</td>
            <td style="padding:5px 8px;text-align:right;">${Math.round(face.As_prov||0)}</td>
            <td style="padding:5px 8px;text-align:right;font-weight:700;color:${accentColor};">${bestBar.label||'&mdash;'}</td>
            <td style="padding:5px 8px;text-align:right;">
                <span style="color:${uColor};font-weight:700;">${utilPct.toFixed(0)}%</span>
            </td>
        </tr>`;
    }
    ...
    if (beamDesigns.length > 0) {
        html += `
        <div style="background:white;border:1px solid #e5e7eb;border-radius:8px;
                    padding:14px 16px;margin-bottom:14px;box-shadow:0 1px 4px rgba(0,0,0,0.06);">
            <div style="font-size:13px;font-weight:700;color:#1e40af;margin-bottom:12px;">
                BEAM DESIGN &mdash; BS 8110:1997 (per member)
            </div>`;

        beamDesigns.forEach(bd => {
            ...
            const h_mm  = sec.h    || Math.round((d.uls||{}).depth || 450);
            const d_eff = sec.d_eff|| Math.round((d.uls||{}).eff_depth || h_mm - 60);
            const bWidth= sec.b    || 225;
            ...
            <div style="margin-bottom:14px;padding-bottom:14px;border-bottom:1px solid #e5e7eb;">
                <div style="font-size:12px;font-weight:700;color:#374151;background:#f0f4ff;
                            padding:6px 10px;border-radius:6px;margin-bottom:10px;
                            display:flex;justify-content:space-between;align-items:center;">
                    <span>Member ${bd.label} &mdash; Beam &nbsp; (${bd.L.toFixed(2)} m)</span>
                    <span style="font-weight:400;color:#6b7280;">${bWidth}&times;${h_mm} mm &nbsp; d = ${d_eff} mm</span>
                </div>

                <table style="width:100%;border-collapse:collapse;font-size:11px;margin-bottom:8px;">
                    <thead><tr style="background:#f9fafb;">
                        <th style="padding:5px 8px;text-align:left;border-bottom:1px solid #e5e7eb;color:#6b7280;">Face</th>
                        <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">M (kNm)</th>
                        <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">K / K'</th>
                        <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">z (mm)</th>
                        <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">As,req</th>
                        <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">As,prov</th>
                        <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">Bars</th>
                        <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">Util.</th>
                    </tr></thead>
                    <tbody>
                        ${faceRow(bot, 'Bottom', '#15803d')}
                        ${faceRow(top, 'Top',    '#b91c1c')}
                    </tbody>
                </table>

                <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px;">
                    <div style="background:#fffbeb;border-left:3px solid ${shOk?WARN:FAIL};
                                border-radius:4px;padding:8px 10px;font-size:11px;">
                        <div style="font-weight:700;color:#92400e;margin-bottom:2px;">Shear</div>
                        <div style="color:#374151;">
                            v = ${(shr.v||0).toFixed(3)} N/mm&sup2; &nbsp;
                            vc = ${(shr.vc||0).toFixed(3)} N/mm&sup2;
                        </div>
                        <div style="color:#374151;margin-top:2px;">
                            Links: <b>${lkBest.label||'R8@nom'}</b> (2-leg) &nbsp;|&nbsp; sv,max = ${(shr.sv_max||0).toFixed(0)} mm
                        </div>
                    </div>
                    <div style="background:${sls.deflection_check==='OK'?'#f0fdf4':'#fef2f2'};
                        ...
                        <div style="color:#374151;">
                            Deflection: <b>${sls.deflection_check||'?'}</b>
                            &nbsp; span/d = ${(sls.span_depth_actual||0).toFixed(1)} / ${(sls.span_depth_allowable||0).toFixed(1)}
                        </div>
                        <div style="color:#374151;margin-top:2px;">
                            Crack: <b>${sls.crack_check||'?'}</b>
                            &nbsp; s = ${(sls.actual_spacing||0).toFixed(0)} mm (max ${(sls.max_bar_spacing||0).toFixed(0)} mm)
                        </div>
                    </div>
                </div>
            </div>`;
    ...
    if (columnDesigns.length > 0) {
        ...
        <div style="background:white;border:1px solid #e5e7eb;border-radius:8px;
            ...
            <div style="font-size:13px;font-weight:700;color:#7c3aed;margin-bottom:12px;">
                COLUMN DESIGN &mdash; BS 8110:1997 (per member)
            </div>
            <table style="width:100%;border-collapse:collapse;font-size:12px;">
                <thead><tr style="background:#f9fafb;">
                    <th style="padding:5px 8px;text-align:left;border-bottom:1px solid #e5e7eb;color:#6b7280;">Member</th>
                    <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">L (m)</th>
                    <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">N (kN)</th>
                    <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">M (kNm)</th>
                    <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">As,req (mm&sup2;)</th>
                    <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">As,prov (mm&sup2;)</th>
                    <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">Bars</th>
                    <th style="padding:5px 8px;text-align:right;border-bottom:1px solid #e5e7eb;color:#6b7280;">Util.</th>
                </tr></thead>
                <tbody>`;
        columnDesigns.forEach(cd => {
            ...
            const util    = (du.utilization || 0).toFixed(0);
            const uColor  = parseFloat(util) < 70 ? PASS : parseFloat(util) < 90 ? WARN : FAIL;
            const slenderness = du.slenderness_category || '';
            html += `<tr style="border-bottom:1px solid #f3f4f6;">
                <td style="padding:5px 8px;font-weight:600;color:#7c3aed;">${cd.label}</td>
                <td style="padding:5px 8px;text-align:right;">${cd.L.toFixed(2)}</td>
                <td style="padding:5px 8px;text-align:right;">${N_kN}</td>
                <td style="padding:5px 8px;text-align:right;">${M_kNm}</td>
                <td style="padding:5px 8px;text-align:right;color:#7c3aed;font-weight:600;">${As_req}</td>
                <td style="padding:5px 8px;text-align:right;">${As_prov}</td>
                <td style="padding:5px 8px;text-align:right;font-weight:600;color:#7c3aed;">${nBars}Y${bDia}</td>
                <td style="padding:5px 8px;text-align:right;font-weight:700;color:${uColor};">${util}%</td>
            </tr>
            ${slenderness ? `<tr style="border-bottom:1px solid #f3f4f6;">
                <td colspan="8" style="padding:3px 8px;font-size:11px;color:#6b7280;">${cd.label}: ${slenderness}</td>
            </tr>` : ''}`;
        });
        html += `</tbody></table></div>`;
    }
    ...
    if (data.member_forces && data.member_forces.length > 0) {
        ...
        <div style="background:white;padding:16px;border-radius:10px;margin-bottom:14px;
            ...
            <div style="font-size:13px;font-weight:700;color:#444;margin-bottom:10px;">Member End Moments &amp; Forces</div>
            <table style="width:100%;border-collapse:collapse;font-size:13px;">
    ...
    if (data.reactions && data.reactions.length > 0) {
        ...
        <div style="background:white;padding:16px;border-radius:10px;margin-bottom:14px;
                    box-shadow:0 1px 6px rgba(0,0,0,0.08);">
            <div style="font-size:13px;font-weight:700;color:#444;margin-bottom:10px;">Support Reactions</div>
            <table style="width:100%;border-collapse:collapse;font-size:13px;">
                <thead><tr style="background:#f7f7f7;">
                    <th style="padding:7px 10px;text-align:left;border-bottom:2px solid #eee;">Node</th>
                    <th style="padding:7px 10px;text-align:right;border-bottom:2px solid #eee;">Rx (kN)</th>
                    <th style="padding:7px 10px;text-align:right;border-bottom:2px solid #eee;">Ry (kN)</th>
    ...
    if (data.diagrams) {
        ...
        <div style="background:white;padding:16px;border-radius:10px;
                    box-shadow:0 1px 6px rgba(0,0,0,0.08);">
            <div style="font-size:13px;font-weight:700;color:#444;margin-bottom:12px;">Diagrams</div>
