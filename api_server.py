...
def create_beam_diagram_image(x, y, title, ylabel, color):
    """Create detailed annotated beam diagram with dark theme"""
    BG     = '#0d1117'
    PANEL  = '#161b22'
    BORDER = '#30363d'
    TEXT   = '#e6edf3'
    ...
    # Pick colours for positive / negative fills
    pos_fill  = '#1f6feb' if 'Moment' in title else '#1b4f72'
    neg_fill  = '#6a0c0c' if 'Moment' in title else '#5d4037'
    line_col  = '#f47067' if 'Moment' in title else '#58a6ff'
    ...
    ax.fill_between(x, 0, y, where=(y >= 0), color=pos_fill, alpha=0.45, interpolate=True, zorder=3)
    ax.fill_between(x, 0, y, where=(y < 0), color=neg_fill,  alpha=0.45, interpolate=True, zorder=3)
    ax.axhline(0, color=BORDER, linewidth=1.8, zorder=2)
    ax.grid(True, alpha=0.18, linestyle=':', linewidth=0.7, color=BORDER, zorder=1)

    # --- Smart annotations: max, min, and end values ---
    def annotate_val(ax, xv, yv, label, above=True):
        offset = 14 if above else -18
        fc = BG
        ax.annotate(
            label,
            xy=(xv, yv), xytext=(0, offset),
            textcoords='offset points',
            fontsize=8, fontweight='bold', ha='center', color=line_col,
            bbox=dict(boxstyle='round,pad=0.3', facecolor=fc, edgecolor=line_col,
                      alpha=0.92, linewidth=0.8),
            arrowprops=dict(arrowstyle='->', color=line_col, lw=0.9)
            if abs(offset) > 15 else {}
        )

    if len(y) > 0:
        idx_max  = int(np.argmax(y))
        idx_min  = int(np.argmin(y))
        max_val  = float(y[idx_max])
        min_val  = float(y[idx_min])

        # Mark global extremes
        if abs(max_val) > 0.01:
            ax.plot(x[idx_max], max_val, 'o', color=line_col, markersize=7, zorder=6)
            annotate_val(ax, x[idx_max], max_val, f'{max_val:.3f}', above=True)
        if abs(min_val) > 0.01 and idx_min != idx_max:
            ax.plot(x[idx_min], min_val, 'o', color='#f0883e', markersize=7, zorder=6)
            ax.annotate(
                f'{min_val:.3f}',
                xy=(x[idx_min], min_val), xytext=(0, -18),
                textcoords='offset points',
                fontsize=8, fontweight='bold', ha='center', color='#f0883e',
                bbox=dict(boxstyle='round,pad=0.3', facecolor=BG, edgecolor='#f0883e',
                          alpha=0.92, linewidth=0.8),
            )
        ...
        for end_i, (xv, yv) in [(0, (x[0], y[0])), (-1, (x[-1], y[-1]))]:
            if abs(yv) > 0.01:
                ax.plot(xv, yv, 's', color='#3fb950', markersize=6, zorder=5)
                ax.annotate(
                    f'{yv:.3f}',
                    xy=(xv, yv), xytext=(12 if end_i == 0 else -12, 10),
                    textcoords='offset points',
                    fontsize=7.5, ha='left' if end_i == 0 else 'right',
                    color='#3fb950',
                    bbox=dict(boxstyle='round,pad=0.25', facecolor=BG,
                              edgecolor='#3fb950', alpha=0.85, linewidth=0.7),
                )

    ax.set_title(title, fontsize=12, fontweight='bold', color=TEXT, pad=10,
                 fontfamily='monospace')
    ax.set_xlabel('Position along beam (m)', fontsize=10, color=MUTED, fontweight='bold')
    ax.set_ylabel(ylabel, fontsize=10, color=MUTED, fontweight='bold')
...
def generate_frame_diagram(nodes, members, diagram_type='bmd'):
    """Generate BMD or SFD image — detailed dark-theme version"""
    BG    = '#0d1117'
    PANEL = '#161b22'
    GRID  = '#30363d'
    TEXT  = '#e6edf3'
    ...
    for member in members:
        ax.plot([member.node_a.x, member.node_b.x],
                ...
                color='#58a6ff', linewidth=4, zorder=1, solid_capstyle='round', alpha=0.7)
    ...
    # Node markers
    for node in nodes:
        ax.plot(node.x, node.y, 'o', color='#3fb950', markersize=9,
                zorder=10, markeredgecolor='#e6edf3', markeredgewidth=1.5)
        ax.annotate(f'  {chr(65 + nodes.index(node))}', xy=(node.x, node.y),
                    fontsize=9, color='#e6edf3', fontweight='bold', zorder=11)
    ...
    for member in members:
        ...
        ax.fill(x_fill, y_fill, color=fill_color, alpha=0.55, zorder=2, edgecolor=color, linewidth=0.5)

        # Dashed reference lines at ends
        ax.plot([x_local[0], diagram_x[0]], [y_local[0], diagram_y[0]],
                linestyle='--', color=GRID, lw=0.8, alpha=0.6, zorder=2)
        ...
        for idx in ann_indices:
            ...
            if key in ann_seen:
                continue
            ann_seen.add(key)
            ann_col = '#f0883e' if idx == peak_idx else '#79c0ff'
            ...
            ax.annotate(f'{v:.2f} {unit_text}',
                        ...
                        bbox=dict(boxstyle='round,pad=0.3', facecolor=BG,
                                  edgecolor=ann_col, alpha=0.9, linewidth=0.8))

    ax.set_title(title_str, fontsize=13, fontweight='bold', color=TEXT, pad=12,
                 fontfamily='monospace')
    ax.set_xlabel('X (m)', fontsize=10, color=MUTED, fontweight='bold')
    ax.set_ylabel('Y (m)', fontsize=10, color=MUTED, fontweight='bold')
    ax.grid(True, alpha=0.18, linestyle=':', linewidth=0.7, color=GRID)
    ax.set_aspect('equal')
